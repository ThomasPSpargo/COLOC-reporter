...
```{r, include=FALSE}
#Provisionally read-in the summary figures and only include them in the report if two traits are being compared [otherwise the plots are simple duplicates of the SNP-wise summaries produced within the  finemapping report]

if(!is.null(parentParams$summaryFigures)){
...
# SNP-wise trait comparisons {.tabset}

The following plots display comparisons between traits across various measures at a per-SNP level. `r ifelse(exists("finemapReport"),"SNPs are coloured according to any credible set assignments made during per-trait finemapping. PIP values were determined during finemapping, whilst the other","These")` measures were obtained from the GWAS summary statistics.
